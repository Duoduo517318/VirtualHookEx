...
public final class VClientImpl extends IVClient.Stub {
    ...
    private void bindApplicationNoCheck(String packageName, String processName, ConditionVariable lock) {
        ...
        appyVirtualHook(packageName, mInitialApplication.getClassLoader());

    }

    private  void appyVirtualHook(String packageName, ClassLoader targetClassLoader){
        try {
            for(String pluginName : VPackageManager.get().getInstalledHookPlugins()) {
                applyHookPlugin(packageName, pluginName, targetClassLoader);
            }
        }
        catch (Exception e) {
            VLog.printStackTrace("VirtualHook");
        }
    }

    private void applyHookPlugin(String packageName,  String pluginName, ClassLoader appClassLoader) {
        VLog.w("VirtualHook", "Applying hook "+pluginName);
        String apkPath = VEnvironment.getPackageResourcePath(pluginName).getAbsolutePath();
        String libPath = VEnvironment.getPackageLibPath(pluginName).getAbsolutePath();
        String optimizedDirectory = VEnvironment.getDalvikCacheDirectory().getAbsolutePath();
        DexClassLoader dexClassLoader = new DexClassLoader(apkPath, optimizedDirectory, libPath, appClassLoader);
        HookMain.doHookDefault(packageName, dexClassLoader, appClassLoader);
