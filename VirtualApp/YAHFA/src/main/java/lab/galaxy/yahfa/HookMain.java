...
public class HookMain {
    ...
    public static void doHookDefault(String packageName , ClassLoader patchClassLoader, ClassLoader originClassLoader) {
        try {
            Class<?> hookInfoClass = Class.forName("lab.galaxy.yahfa.HookInfo", true, patchClassLoader);
            Object[] hookInfoItems  = (Object[]) hookInfoClass.getField("hookInfoItems").get(null);
            for (Object hookInfoItem : hookInfoItems){

                String   hookPackageName = (String) ReflectUtil.self().getObjectFiled(hookInfoItem ,"hookPackageName");
                String[] hookItemNames   = (String[]) ReflectUtil.self().getObjectFiled(hookInfoItem ,"hookItemNames");

                Log.i(TAG, "packagename: "+packageName + "  hookPackagename:"+hookPackageName);

                if(packageName.equals(hookPackageName)){
